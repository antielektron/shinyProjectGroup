...
Wir wollen eine Schattentechnik verwenden, die möglichst hohe Qualität liefert und dabei performant bleibt. Wünschenswert, wäre ebenfalls filterbare Shadow Maps und eine performante Methode. Moment Shadow Mapping liefert uns dieses Verfahren und wird im weiteren Verlauf erläutert und implementiert. Uns war es wichtig das wir diese Technik in OpenGL und C++ verwirklichen können. Moment Shadow Mapping basiert auf den Variance Shadow Maps, welches wiederum auf PCF basiert, bzw. dieses weiter verbessert. Deswegen schauen wir uns im Related Work Teil auch diese Techniken an und klären im weiteren Verlauf, warum Moment Shadow Maps bessere Qualität liefern.
Desweiteren versuchen wir Volumetric Obscurance mit Momenten zu versehen und so einen Term für die Volumetric Obscurance zu berechnen.
...
Moment Based Volumetric Obscurance \cite{mbvo}
...
Percentage Closer Filtering sorgt bereits für filterbare Shadow Maps. Allerdings nur je Fragment, da dies beim Tiefentest geschiet.

Variance Shadow Maps benutzt 2 Momente und löst das General Moment Problem. Ein filtern ist ebenfalls möglich und PCF wird weiter verbessert bzw. approximiert. VSM ist hinsichtlich der MSM ein spezialfall für $m = 2$.
...
Wir nutzen den Algorithmus Hamburger Moment Shadow Mapping. Dieser versucht das General Moment Problem zu lösen. Hierfür nutzen wir die Algorithmus 3 aus dem Moment Shadow Mapping Paper.\cite{msm} Der grundlegende Aufbau des MSM, ist dem der VSM sehr ähnlich, denn VSM ist hinsichtlich des MSM ein Spezialfall für $m = 2$. Für uns wünschenswert, war aber eine Methode mit $m = 4$.
Wir möchten das General Moment Problem lösen, damit wir eine möglichst scharfe untere Schranke nutzen können.\cite{msm}. 
Auch VSM löst dieses und der Vorteil liegt in der Vermeidung von Artefakten, wie etwa Surface acne. Übrig bleiben noch Artefakte wie Light Leeking durch falsche Schattenintensitä. Da wir bisher eine untere Schranke haben überschätzen wird niemals die Werte niemals für die Schattenintensität. Allerdings können nocht Fehler wie etwa Light Leeking auftreten. 
Eine weitere Vereinfachung in Richtung Hamburger MSM wird im Paper durchdurch Algorithmus 2 erreicht, welcher $F_s(z_f)$ zu minimiert. Desweiteren bestimmen wir Gewichte durch den Algorithmus 2 durch $E_s(b) = b$ und $G(b,z_f) = F_s(z_f)$.
Algorithmus 2 bietet aber noch nicht die nötige Robustheit für eine Echtzeitanwendung.\cite{msm}
Letztendlich führt uns dies zu Algorithmus 3, welchen wir auch genutzt haben. Dieser ist ein Spezialfall des Algorithmus 2 und liefert und  die gewünschte Schattenintensität. 
Light Leeking und Aliasing werden vermindert und wir erhalten eine Schattenintensität, die sowohl helle als auch dunkle Werte gut einschätzen kann. Dies übertrifft die Ergebnisse von VSM und PCF offensichtlich. Ausserdem ist dieser Algorithmus robust genug für Echtzeitanwendung, was in der Implementierung aufgezeigt wird. 
