...
int samples = 100; //TODO, pass through shader
float ovMultiplier = 1.7;
float ovFactor = 2;
float defaultFactor = 0.5;
...
{
	float z = (texture2D(sampler, uv).a);
	vec3 defaultCol = texture2D(sampler, uv).xyz;
	vec3 ovCol = lineSampling(samples) * ovMultiplier * ovFactor * vec3(1.,1.,1.);
	vec3 color = texture2D(sampler, uv).xyz * defaultFactor;
    outputColor = vec4(color * ovCol * ovCol , 1.);
}
