...
void main()
{
    ...
    float Fcoef = 2.0 / log2(cascadeFar[3] + 1.0);
	float z = log2(max(1e-6, 1.0 + gl_FragCoord.w)) * Fcoef - 1.0;
	float flogz = 1.0 + gl_FragCoord.w;
	

    gl_FragDepth = 1 - log2(flogz) * Fcoef * 0.5;
}
